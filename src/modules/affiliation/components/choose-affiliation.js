...
export default function ChooseAffiliation() {
  const { defaultAffiliation, affiliationOptions } = useSelector(state => state.affiliation);
  ...
  function changeAffiliation() {
    const parsed = qs.parse(document.location.search.substring(1), { allowDots: true });
    const withAffiliation = {
      ...parsed,
      affiliation: alternativeAffiliation
    }

    document.location.href = document.location.pathname + '?' + qs.stringify(
      withAffiliation,
    {
      arrayFormat: 'repeat',
    ...
    <React.Fragment>
      ...
      >
        <div css={{
          ...
          <Text>Selecting an affiliation helps us connect you to available online materials licensed for your campus.</Text>
          ...
          <Button kind="secondary" onClick={changeAffiliation} role="link">Change to {alternativeLabel}</Button>

          <Text css={{ marginBottom: '0', color: COLORS.neutral['300'] }} small>You can still use Library Search if you're not affiliated with either campus.</Text>
        </div>
      </Modal>
    </React.Fragment>
  )
